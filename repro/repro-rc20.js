...
ok(client.includes('bar.firstElementChild'), 'client bar measure uses entry root (outlet wrapper is display:contents, rect=0)')
ok(client.includes('display:contents'), 'client documents display:contents root cause')
ok(client.includes('const barRoot = bar && bar.firstElementChild'), 'client barRoot fallback guard')
ok(client.includes("if (!width) {"), 'client degenerate rect falls through to seat')
...
ok(client.includes("t('call.label')"), 'client idle call label usage')
ok(client.includes('Object.assign({}, pillStyle, effective'), 'client voice pill shares pillStyle (same width)')
ok(client.includes('expectW'), 'client card width expectation in measure')
ok(client.includes('cardEl'), 'client card element detection in measure')
